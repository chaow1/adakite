...
Innerfactor <-
  length(grep(Estimate_Pack, Check_Package))

if (Innerfactor == 0) {
  print("You do not install rJava, the chart will be saved as .csv file")
}
...
if (length(grep("regions", excel_name)) == 1) {
  ...
  set_shape = c(25, 21, 21, 25, 24, 25, 24, 25, 24)
}
...
#Rock.Component <- read.csv(set_datafile_location)
...
individual_coordinate <-
  cbind(Rock.Data.class, individual_coordinate[, 1:3])


names(individual_coordinate) <-
  c("Subtype", "Dim.1", "Dim.2", "Dim.3")
...
Sheet_Name <- paste0(excel_name, " PCA result", ".xlsx")
write.xlsx2(individual_coordinate, Sheet_Name, "samples coordinate")
write.xlsx2(eigenvalue,
            Sheet_Name,
            "eigenvalue(explanation) for principle components",
            append = TRUE) #ע˱comp == dim
write.xlsx2(varr, Sheet_Name, "Variances_New Corrod Sys", append = TRUE)
print("complete")
...
vect1_2 <- ggplot() +
  ...
  geom_segment(
    ...
    mapping = aes(
      ...
      xend = Dim.1,
      yend = Dim.2,
      color = vect1_2.length              #coloring the segment by length
    ),
    arrow = arrow(length = unit(0.25, 'picas')),
    #add arrow to segment to form vector
    size = 0.25
  ) +
  geom_text_repel(
    #label the vector by element
    data = varr,
    aes(
      x = Dim.1,
      y = Dim.2,
  ...
  scale_color_gradientn(colors = set_color_vector,
                        name = c("cos2")) +
...
pdf(paste0(excel_name, " PCA_Vec.pdf"),
    width = 10,
    height = 5)
...
pc1_2_all.scatter <-
  ...
  geom_point(aes(fill = Subtype, shape = Subtype), size = 3) +
...
pc1_3_all.scatter <-
  ...
  geom_point(aes(fill = Subtype, shape = Subtype), size = 3) +
  scale_fill_manual(values = set_color) +  #color
  scale_shape_manual(values = set_shape) + #shape
  labs(
    x = paste("PC1 / ", format(PC1_Num, digits = 4), "%", sep = ""),
...
set_color_deposit =  c("#FFFF33",
                       ...
                       set_color_mature =  c(# "#BEBADA",
                         ...
                         #### output figures in pdf & svg and data in xlsx ####
