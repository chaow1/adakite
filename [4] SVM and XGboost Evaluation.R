...
data1 <- c("PCA data")              # original data or imputed data or PCA data   
data2 <- c("crust")               # regions or crust
data3 <- c("filtered elements")         # filtered elements or all elements

SamplingMethod <- c("Balance")      # Strata or Overall or Balance
...
output_file_name <- paste(data1, data2, data3, SamplingMethod)
...
if (data2 == c("crust") && data3 == c("filtered elements")) {
  excluded_elements = c("Pr", "Dy", "Fe2O3", "FeO", "P2O5", "Eu", "Gd", "Tb", "Hf")
  element_gather <- setdiff(element_gather, excluded_elements)
}
if (data2 == c("regions") && data3 == c("filtered elements")) {
...
set.seed(1) 
RF.model <- randomForest(Subtype ~ ., data = traindb, type = classification,  probability = TRUE, importance =TRUE)
RF.pred <- predict(RF.model, testsdb[,-1], type = "prob") 

saveRDS(RF.model,file = paste0("randomForest model for ", output_file_name, ".rds")) 
RF.pred1 <- predict(RF.model, testsdb[,-1], type = "class")        
RF.table <- table(RF.pred1, testsdb$Subtype)                
...
table_Name <- paste0("xgboost data for ", output_file_name, " ", Reduce(paste, colnames(Rock.Data)),".xlsx")
...
if(data2 == c("crust")){
xgb.shap.sum.0 <- xgb.shap.sum.0  + theme(legend.position = "none")
xgb.shap.sum.1 <- xgb.shap.sum.1  + theme(legend.position = c(0.85, 0.15), legend.background = element_rect(fill = rgb(1, 1, 1, 0.5)))
xgb.shap.sum.2 <- xgb.shap.sum.2  + theme(legend.position = "none")
xgb.shap.sum.3 <- xgb.shap.sum.3  + theme(legend.position = c(0.85, 0.15), legend.background = element_rect(fill = rgb(1, 1, 1, 0.5)))
}
if(data2 == "regions"){
  xgb.shap.sum.0 <- xgb.shap.sum.0  + theme(legend.position = "none")
  xgb.shap.sum.1 <- xgb.shap.sum.1  + theme(legend.position = c(0.18, 0.15), legend.background = element_rect(fill = rgb(1, 1, 1, 0.5)))
  xgb.shap.sum.2 <- xgb.shap.sum.2  + theme(legend.position = "none")
  xgb.shap.sum.3 <- xgb.shap.sum.3  + theme(legend.position = "none")
  xgb.shap.sum.4 <- xgb.shap.sum.4  + theme(legend.position = c(0.15, 0.15), legend.background = element_rect(fill = rgb(1, 1, 1, 0.5)))
  xgb.shap.sum.5 <- xgb.shap.sum.5  + theme(legend.position = "none")
  xgb.shap.sum.6 <- xgb.shap.sum.6  + theme(legend.position = "none")
  xgb.shap.sum.7 <- xgb.shap.sum.7  + theme(legend.position = c(0.18, 0.15), legend.background = element_rect(fill = rgb(1, 1, 1, 0.5)))
}
pdf(
...
for (i in (1:length(class_level))) {             
  ...
  shap_svm_importance <- shap_svm_importance[c(1:No_important_elements),]
    ...
    as.data.frame() %>%
    mutate(id = 1:n())%>%
    pivot_longer(cols = -ncol(traindb), values_to = "shap") # ̱
...
if(data2 == c("crust")){
svm.shap.sum.0 <- svm.shap.sum.0  + theme(legend.position = "none")
svm.shap.sum.1 <- svm.shap.sum.1  + theme(legend.position = c(0.83, 0.22), legend.background = element_rect(fill = rgb(1, 1, 1, 0.5)))
svm.shap.sum.2 <- svm.shap.sum.2  + theme(legend.position = "none")
svm.shap.sum.3 <- svm.shap.sum.3  + theme(legend.position = c(0.83, 0.22), legend.background = element_rect(fill = rgb(1, 1, 1, 0.5)))
}
if(data2 == "regions"){
  svm.shap.sum.0 <- svm.shap.sum.0  + theme(legend.position = "none")
  svm.shap.sum.1 <- svm.shap.sum.1  + theme(legend.position = c(0.80, 0.22), legend.background = element_rect(fill = rgb(1, 1, 1, 0.5)))
  svm.shap.sum.2 <- svm.shap.sum.2  + theme(legend.position = "none")
  svm.shap.sum.3 <- svm.shap.sum.3  + theme(legend.position = "none")
  svm.shap.sum.4 <- svm.shap.sum.4  + theme(legend.position = c(0.84, 0.22), legend.background = element_rect(fill = rgb(1, 1, 1, 0.5)))
  svm.shap.sum.5 <- svm.shap.sum.5  + theme(legend.position = "none")
  svm.shap.sum.6 <- svm.shap.sum.6  + theme(legend.position = "none")
  svm.shap.sum.7 <- svm.shap.sum.7  + theme(legend.position = c(0.2, 0.22), legend.background = element_rect(fill = rgb(1, 1, 1, 0.5)))
}
pdf(
...
load(file=paste0(output_file_name, " xgboost and svm shap with ", simu_No," simulation",  ".RData"))
  ...
  if (predict_model == c("randomForest")) {
    Predicted_range <- predict(RF.model, SVMmodelGrid)
    } 
  if (predict_model == c("xgboost")){
    Predicted_range <- predict(model_xgb, SVMmodelGrid) 
  }
