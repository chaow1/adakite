###########################################
### MICE Imputation and clr norm Script ###
###########################################
###########################################
...
temp.mice.comb[, c(k:(k + Num.major.element - 1))] <-
  mice.comb[, c(k:(k + Num.major.element - 1))] * 10000 

temp.original.comb[, c(k:(k + Num.major.element - 1))] <-
  original.comb[, c(k:(k + Num.major.element - 1))] * 10000 


mice.comb.clr <-
  compositions::clr(temp.mice.comb[, c(k:(k + Num.major.element + Num.trace.element - 1))])      

original.comb.clr <-
  compositions::clr(temp.original.comb[, c(k:(k + Num.major.element + Num.trace.element - 1))])      
...
write.xlsx2(miss.data.comb[,-1], paste0(table_name, " imputed data.xlsx"), "precentage of missing data") 
write.xlsx2(mice.comb.clr, paste0(table_name, " imputed data.xlsx"), "imputed data by MICE and clr", append = TRUE) 
write.xlsx2(mice.comb, paste0(table_name, " imputed data.xlsx"), "imputed data by MICE", append = TRUE) 

write.xlsx2(original.comb.clr, paste0(table_name, " original data.xlsx"), "original data by clr") 
print("===complete===")
